...
export class AuthProvider extends React.Component {
  ...
  componentDidMount() {
    axios.get('https://tomato-mart.herokuapp.com/user', { headers: {"Authorization" : `Bearer ${this.state.token}`}}, { cancelToken: source.token })
         .then(res => {
            this.setState({
              user: res.data,
              loading: false
  ...
  userLogin(token = '') {
    axios.get('https://tomato-mart.herokuapp.com/user', { headers: {"Authorization" : `Bearer ${token}`}} )
    .then(res => {
      this.setState({
        token: token,
        user: res.data,
