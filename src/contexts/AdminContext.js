...
export class AdminProvider extends React.Component {
  ...
  async componentDidMount() {
    ...
    axios.get('https://tomato-mart.herokuapp.com/admin/admin-get', { headers: {"Authorization" : `Bearer ${adminToken}`}}, { cancelToken: source.token })
         .then(res => {
           this.setState({
    ...
    try {
      const response = await axios.get('https://tomato-mart.herokuapp.com/promotion');
      this.setState({
  ...
  adminLogin(token = '') {
    axios.get('https://tomato-mart.herokuapp.com/admin/admin-get', { headers: {"Authorization" : `Bearer ${token}`}}, { cancelToken: source.token })
         .then(res => {
           this.setState({
             adminToken: token,
  ...
  render() {
    ...
    if (filter.payment !== 'Phương thức thanh toán') {
      const paymentMethod = filter.payment === 'Thanh toán khi nhận hàng' ? 'cash' : 'card';
      filtedOrders = filtedOrders.filter(order => {
        return order.payment === paymentMethod;
      })
    }

    if (filter.amount !== 'Số tiền') {
      if (filter.amount === 'Cao đến thấp') {
    ...
    if (filter.orderAmount !== 'Số đơn') {
      if (filter.orderAmount === 'Cao đến thấp') {
