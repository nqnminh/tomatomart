...
export class ProductsProvider extends React.Component {
  ...
  getProducts() {
    ...
    axios.get(`https://tomato-mart.herokuapp.com/products/?${paramsString}`, { cancelToken: source.token })
      .then(res => {
        this.setState({
          products: res.data,
          isLoading: false,
  ...
  getCategory() {
    axios.get(`https://tomato-mart.herokuapp.com/category`, { cancelToken: source.token })
      .then(res => {
        let data = res.data;
        this.setState({
          categoryLists: data
        },() => console.log('popop', this.state.categoryLists))
      })
      .catch(err => {
        console.log(err);
      })
  ...
  setProduct(title = '') {
    axios.get(`https://tomato-mart.herokuapp.com/products/product/?slug=${title}`, { cancelToken: source.token })
      .then(res => {
        this.setState({
          product: res.data,
        })
      ...
      .then(() => {
        axios.get(`https://tomato-mart.herokuapp.com/products/?_category=${this.state.product.category}`, { cancelToken: source.token })
          .then(res => {
            this.setState({
  ...
  render() {
    const { displayCategory, keyword, filters, isLoading, products, product, relatedItems, isLoadMore ,categoryLists} = this.state;
